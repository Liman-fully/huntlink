...
export class CandidateController {
  ...
  @ApiQuery({ name: 'limit', required: false, description: '每页数量', default: 20 })
  @ApiQuery({ name: 'sortBy', required: false, description: '排序字段', default: 'relevance' })
  @ApiQuery({ name: 'sortOrder', required: false, description: '排序方式', default: 'DESC' })
  ...
  @ApiQuery({ name: 'limit', required: false, description: '建议数量', default: 5 })
  ...
  async highlightResult(@Query('keyword') keyword: string, @Query('id') id: number) {
    const result = await this.candidateService.highlightResult(id, keyword);
    return {
      success: true,
      data: result,
    };
